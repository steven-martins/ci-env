...
ENV DEBIAN_FRONTEND noninteractive
RUN apt-get install -y mysql-server mysql-client libmysqlclient-dev --no-install-recommends && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
